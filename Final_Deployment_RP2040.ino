...
bool applyDcraToWav(const WavInfo& info, FsFile& inFile, FsFile& outFile, float biasWindowSec) {
  // Applies centered moving-average DC removal to a mono 16-bit PCM WAV file.
  // The filter removes slow DC bias and very low-frequency drift by subtracting a
  // local moving average from each sample.
  //
  // Key properties of this implementation:
  // - byte-safe: all sample access is done through uint8_t buffers
  // - fixed-buffer: no heap allocation in the hot path
  // - rolling-sum interior update: avoids recomputing the full window every sample
  // - reflection padding at the edges: prevents edge discontinuities and avoids a
  //   separate zero-pad branch
  //
  // The function reads the input WAV data in blocks, processes each block, writes
  // the filtered samples to the output WAV, and then patches the header sizes.
  //
  // The active deployment target is strictly:
  // - mono
  // - 16-bit PCM
  // - blockAlign == 2
  //
  // Returns true on success and false on any format, I/O, or buffer-size failure.
  ...
  if (kDebugPipeline && Serial) {
    Serial.println(F("[dcra] copying WAV header"));
  }
